...
from app.services.orders import create_product_order, pay_order_with_credits
...
class ProductTypeSelect(discord.ui.Select):
    def __init__(self, product_types: list[str]) -> None:
        labels = {"item": "Itens", "robux": "Robux", "gamepass": "Game Pass"}
        options = [
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        if not products:
            await interaction.response.edit_message(content="Nenhum produto disponível nessa categoria.", view=None)
            return
        await interaction.response.edit_message(
...
class StoreHomeView(discord.ui.View):
    ...
    async def buy(self, interaction: discord.Interaction, _: discord.ui.Button) -> None:
        ...
        if not product_types:
            await interaction.response.send_message("A loja ainda não tem produtos ativos.", ephemeral=True)
            return
